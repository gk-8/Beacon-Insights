...
namespace UniversalBeaconLibrary.Beacon
{
    ...
    public class Beacon : INotifyPropertyChanged
    {
        ...
        public enum ProximityRangeEnum
        {
            Unknown,
            Immediate,
            Near,
        ...
        public enum ProximityStatusEnum
        {
            NotMoving,
            GettingCloser,
            GettingFurther
        ...
        private ProximityRangeEnum _proximityRange;
        public ProximityRangeEnum ProximityRange
        {
            get { return _proximityRange; }
            set
            {
                if (_proximityRange == value) return;
                _proximityRange = value;
                OnPropertyChanged();
            }
        }

        private ProximityStatusEnum _proximityStatus;
        public ProximityStatusEnum ProximityStatus
        {
            get { return _proximityStatus; }
            set
            {
                if (_proximityStatus == value) return;
        ...
        public void UpdateBeacon(BluetoothLEAdvertisementReceivedEventArgs btAdv)
        {
            ...
            if (btAdv.Advertisement.ManufacturerData.Any())
            {
                foreach (var manufacturerData in btAdv.Advertisement.ManufacturerData)
                {
                    ...
                    var manufacturerDataArray = manufacturerData.Data.ToArray();
                    if (BeaconFrameHelper.IsProximityBeaconPayload(manufacturerData.CompanyId, manufacturerDataArray))
                    {
                        BeaconType = BeaconTypeEnum.iBeacon;
                        //Debug.WriteLine("iBeacon Frame: " + BitConverter.ToString(manufacturerDataArray));

                        var beaconFrame = new ProximityBeaconFrame(manufacturerDataArray);
            ...
            foreach (var beaconFrame in BeaconFrames)
            {
                beaconFrame.PropertyChanged -= BeaconFrame_PropertyChanged;
                beaconFrame.PropertyChanged += BeaconFrame_PropertyChanged;
            }
        }

        private void BeaconFrame_PropertyChanged(object sender, PropertyChangedEventArgs e)
        {
            CalculateDistance();
        }
            ...
            if (BeaconType == BeaconTypeEnum.Eddystone)
            {
                foreach (var frame in BeaconFrames)
                {
                    if (frame is UidEddystoneFrame)
                        ...
                        txPower = Convert.ToDouble((frame as UrlEddystoneFrame).RangingData) - 41;
                }

            }
            else if (BeaconType == BeaconTypeEnum.iBeacon)
            {
                if (BeaconFrames.Count > 0 && BeaconFrames[0] is ProximityBeaconFrame)
                    txPower = (BeaconFrames[0] as ProximityBeaconFrame).TxPower;
            }
        ...
        private void CalculateProximityRange()
        {
            ...
            CalculateProximityStatus(previousProximityRange);
        }

        private void CalculateProximityStatus(ProximityRangeEnum previousProximityRange)
        {
            if (ProximityRange != previousProximityRange && previousProximityRange != ProximityRangeEnum.Unknown && ProximityRange != ProximityRangeEnum.Unknown)
            {
                if (previousProximityRange == ProximityRangeEnum.Far)
                    ProximityStatus = ProximityStatusEnum.GettingCloser;
                else if (previousProximityRange == ProximityRangeEnum.Near)
                {
                    if (ProximityRange == ProximityRangeEnum.Far) ProximityStatus = ProximityStatusEnum.GettingFurther;
                    else if (ProximityRange == ProximityRangeEnum.Immediate) ProximityStatus = ProximityStatusEnum.GettingCloser;
                }
                else if (previousProximityRange == ProximityRangeEnum.Immediate)
                    ProximityStatus = ProximityStatusEnum.GettingFurther;
            }
            else
